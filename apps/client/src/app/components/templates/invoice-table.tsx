      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="flex flex-row justify-end gap-5">
            ...
            {row.original.type === 'QUOTE' &&
              row.original.status === 'WAITING_VALIDATION' && (
                <Tooltip>
                  <TooltipContent>{t('devis.accept')}</TooltipContent>
                  <ValidateQuoteDialog
                    trigger={
                      <TooltipTrigger>
            ...
            {row.original.type === 'INVOICE' &&
              row.original.status === 'WAITING_PAYED' && (
                <Tooltip>
                  <TooltipContent>{t('invoice.indicatePay')}</TooltipContent>
                  <TooltipTrigger>
                    <Banknote
                      size={15}
            ...
            {row.original.type === 'QUOTE' &&
              row.original.status === 'WAITING_VALIDATION' && (
                <Tooltip>
                  <TooltipContent>{t('devis.refuse')}</TooltipContent>
                  <TooltipTrigger>
                    <TicketX
                      size={15}
                      onClick={() =>
                        handleValidateQuote(row.original.id, false)
                      }
                    />
                  </TooltipTrigger>
                </Tooltip>
