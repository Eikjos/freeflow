...
import NotificationService from 'private/notifications/notification.service';
import OpinionService from 'private/opinions/opinion.service';
...
export default class EnterprisesController {
  ...
  @UseGuards(EnterpriseGuard)
  @Get(':id/get-api-token')
  getApiToken(@Param('id', ParseIntPipe) id: number, @Req() req: Request) {
    const enterpriseId = parseInt(req.user['enterpriseId']);
    if (enterpriseId !== id) throw new ForbiddenException();
    return this.enterpriseService.getTokenAPI(enterpriseId);
  }
}
