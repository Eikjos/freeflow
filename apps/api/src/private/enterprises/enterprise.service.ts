...
import ExpenseService from 'private/expenses/expense.service';
import MailingService from 'private/mailing/mailing.service';
...
export default class EnterpriseService {
  ...
  async createEnterprise(
    ...
    const enterprise = await this.prisma.enterprise.create({
      data: {
        ...
        mediaId: null,
        apiPublicToken: this.generateApiToken(),
        sales: {
          create: {
            number: 0,
  ...
  async getTokenAPI(enterpriseId: number) {
    const enterprise = await this.prisma.enterprise.findFirst({
      where: { id: enterpriseId },
    });
    if (!enterprise) throw new ForbiddenException();
    return enterprise.apiPublicToken;
  }
  ...
  private generateApiToken() {
    return 'freeflow-' + randomBytes(32).toString('hex');
  }
}
